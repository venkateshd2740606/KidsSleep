package com.kidssleep.data.mapper

import com.google.gson.Gson
import com.google.gson.reflect.TypeToken
import com.kidssleep.data.local.database.entity.AchievementEntity
import com.kidssleep.data.local.database.entity.ChallengeEntity
import com.kidssleep.data.local.database.entity.EconomyEntity
import com.kidssleep.data.local.database.entity.GameEntity
import com.kidssleep.data.local.database.entity.ProfileEntity
import com.kidssleep.data.local.database.entity.StatsEntity
import com.kidssleep.domain.model.PuzzleArchetype
import com.kidssleep.domain.model.PuzzleProfile
import com.kidssleep.domain.model.PuzzleProfileMetrics
import com.kidssleep.domain.model.SkillCategory
...
object DataMappers {
    ...
    private val storyListType = object : TypeToken<List<Story>>() {}.type
    private val intSetType = object : TypeToken<Set<Int>>() {}.type

    fun toEntity(game: KidsSleepGame): GameEntity {
        val gameState = GameStateJson(
            currentStoryIndex = game.currentStoryIndex,
            currentLineIndex = game.currentLineIndex,
            completedStoryIndices = game.completedStoryIndices.toList()
        )
        return GameEntity(
            id = game.id,
            ...
            selectedTubeId = game.currentStoryIndex,
            moves = game.moves,
            ...
            levelJson = gson.toJson(LevelJson(stories = game.level.stories)),
            coinsEarned = game.coinsEarned,
            xpEarned = game.xpEarned
        )
    }

    fun fromEntity(entity: GameEntity): KidsSleepGame {
        ...
        val stories: List<Story> = levelJson?.stories
            ?: levelJson?.let { gson.fromJson(entity.levelJson, LegacyLevelJson::class.java)?.rhymes }
            ?: gson.fromJson(entity.levelJson, storyListType)
            ?: emptyList()

        val level = KidsSleepLevel(
            id = entity.id,
            seed = entity.seed,
        ...
        return if (gameState != null) {
            KidsSleepGame(
                ...
                completedStoryIndices = gameState.completedStoryIndices.toSet(),
            ...
            KidsSleepGame(
                ...
                level = level,
                status = GameStatus.valueOf(entity.status),
                currentStoryIndex = entity.selectedTubeId.coerceAtLeast(0),
                currentLineIndex = 0,
                hintsUsed = entity.hintsUsed,
                moves = entity.moves,
                elapsedSeconds = entity.elapsedSeconds,
    ...
    data class LevelJson(val stories: List<Story>)

    private data class LegacyLevelJson(val rhymes: List<Story>)

    data class GameStateJson(
        val currentStoryIndex: Int,
        val currentLineIndex: Int,
        val completedStoryIndices: List<Int> = emptyList()
    )
}
