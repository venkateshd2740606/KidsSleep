package com.kidssleep.data.repository
...
import com.kidssleep.data.local.database.entity.AchievementEntity
import com.kidssleep.data.mapper.DataMappers
...
import com.kidssleep.engine.PuzzleProfileEngine
import com.kidssleep.engine.KidsSleepEngine
import com.kidssleep.engine.KidsSleepGenerator
import com.kidssleep.engine.TutorialLevels
import com.kidssleep.util.DateUtils
...
class GameRepositoryImpl @Inject constructor(
    ...
    override suspend fun createNewGame(difficulty: Difficulty, levelNumber: Int): KidsSleepGame {
        ...
        val seed = KidsSleepGenerator.seedFromLevelNumber(levelNumber, difficulty)
        val level = KidsSleepGenerator.generate(seed, levelNumber, difficulty, generationProfile)
        return KidsSleepEngine.createInitialGame(level)
    }
    ...
    override suspend fun createTutorialGame(tutorialIndex: Int): KidsSleepGame? {
        val level = TutorialLevels.getTutorialLevel(tutorialIndex) ?: return null
        return KidsSleepEngine.createInitialGame(level)
    }

    override suspend fun createEndlessGame(wave: Int): KidsSleepGame {
        val seed = wave.toLong() * 7919L + System.currentTimeMillis() % 1000
        val level = KidsSleepGenerator.generate(seed, wave, Difficulty.ENDLESS)
        return KidsSleepEngine.createInitialGame(level)
    }

    override suspend fun saveGame(game: KidsSleepGame): Long {
    ...
    override suspend fun getGame(gameId: Long): KidsSleepGame? =
        gameDao.getById(gameId)?.let { DataMappers.fromEntity(it) }

    override suspend fun getInProgressGame(): KidsSleepGame? =
        gameDao.getInProgress()?.let { DataMappers.fromEntity(it) }

    override fun observeInProgressGame(): Flow<KidsSleepGame?> =
        gameDao.observeInProgress().map { it?.let { e -> DataMappers.fromEntity(e) } }

    override suspend fun completeGame(game: KidsSleepGame): KidsSleepGame {
    ...
    override suspend fun getLevel(seed: Long, levelNumber: Int, difficulty: Difficulty): KidsSleepLevel =
        KidsSleepGenerator.generate(seed, levelNumber, difficulty)
}
...
class ChallengeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChallengeGame(record: ChallengeRecord): KidsSleepGame {
        val levelNumber = record.key.hashCode().and(0x7FFFFFFF) % 5000 + 1
        val level = KidsSleepGenerator.generateForChallenge(
...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateStatsAfterGame(game: KidsSleepGame) {
    ...
    private suspend fun updatePuzzleProfile(game: KidsSleepGame) {
    ...
    private fun updateFastest(current: Long, game: KidsSleepGame, difficulty: Difficulty): Long {
...
class PreferencesRepositoryImpl @Inject constructor(
    private val dataStore: com.kidssleep.data.local.PreferencesDataStore
) : com.kidssleep.domain.repository.PreferencesRepository {

    override fun getUserPreferences() = dataStore.preferencesFlow

    override suspend fun updatePreferences(
        transform: (com.kidssleep.domain.model.UserPreferences) -> com.kidssleep.domain.model.UserPreferences
