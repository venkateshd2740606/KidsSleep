package com.kidssleep.domain.model
...
data class Story(
    ...
    init {
        require(lines.isNotEmpty()) { "Each story must have at least one line" }
    }
...
data class GenerationProfile(
    val storySpreadModifier: Int = 0
)

data class KidsSleepLevel(
    val id: Long = 0,
    val seed: Long,
    val levelNumber: Int,
    ...
    val totalLines: Int get() = stories.sumOf { it.lines.size }
}

data class KidsSleepGame(
    val id: Long = 0,
    val level: KidsSleepLevel,
    val status: GameStatus = GameStatus.IN_PROGRESS,
    val currentStoryIndex: Int = 0,
    val currentLineIndex: Int = 0,
    val completedStoryIndices: Set<Int> = emptySet(),
    val moves: Int = 0,
    val hintsUsed: Int = 0,
    ...
    val currentStory: Story? get() = level.stories.getOrNull(currentStoryIndex)
    val currentLine: String? get() = currentStory?.lines?.getOrNull(currentLineIndex)
    val linesRead: Int get() {
        var count = completedStoryIndices.sumOf { level.stories.getOrNull(it)?.lines?.size ?: 0 }
